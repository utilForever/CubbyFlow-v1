...
CUBBYFLOW_BEGIN_TEST_F(GridFluidSolver2, ApplyBoundaryConditionWithPressure)
{
	...
	SurfaceToImplicit2Ptr sphere = std::make_shared<SurfaceToImplicit2>(std::make_shared<Sphere2>(domain.MidPoint(), 0.25));
...
CUBBYFLOW_BEGIN_TEST_F(GridFluidSolver2, ApplyBoundaryConditionWithVariationalPressure)
{
	GridFluidSolver2 solver;
	solver.SetGravity(Vector2D(0, 0));
	solver.SetAdvectionSolver(nullptr);
...
CUBBYFLOW_BEGIN_TEST_F(GridFluidSolver2, ApplyBoundaryConditionWithPressureOpen)
{
	...
	GridSinglePhasePressureSolver2Ptr ppe = std::make_shared<GridSinglePhasePressureSolver2>();
	solver.SetPressureSolver(ppe);
