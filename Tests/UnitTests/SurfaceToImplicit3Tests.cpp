...
TEST(SurfaceToImplicit3, Intersects)
{
	...
	bool result0 = s2i.Intersects(Ray3D(Vector3D(1, 4, 5), Vector3D(-1, -1, -1).Normalized()));
	EXPECT_TRUE(result0);

	bool result1 = s2i.Intersects(Ray3D(Vector3D(1, 2.5, 6), Vector3D(-1, -1, 1).Normalized()));
	EXPECT_TRUE(result1);

	bool result2 = s2i.Intersects(Ray3D(Vector3D(1, 1, 2), Vector3D(-1, -1, -1).Normalized()));
