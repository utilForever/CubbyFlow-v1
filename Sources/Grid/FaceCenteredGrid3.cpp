/*************************************************************************
> File Name: FaceCenteredGrid3.cpp
> Project Name: CubbyFlow
> Author: Chan-Ho Chris Ohk
> Purpose: 2-D face-centered (a.k.a MAC or staggered) grid.
...
namespace CubbyFlow
{
	FaceCenteredGrid3::FaceCenteredGrid3() :
		m_dataOriginU(0.0, 0.5, 0.5), m_dataOriginV(0.5, 0.0, 0.5), m_dataOriginW(0.5, 0.5, 0.0),
		m_uLinearSampler(
	...
	FaceCenteredGrid3::ConstScalarDataAccessor FaceCenteredGrid3::GetUConstAccessor() const
	{
	...
	FaceCenteredGrid3::ConstScalarDataAccessor FaceCenteredGrid3::GetVConstAccessor() const
	{
	...
	FaceCenteredGrid3::ConstScalarDataAccessor FaceCenteredGrid3::GetWConstAccessor() const
	{
	...
	void FaceCenteredGrid3::Fill(const Vector3D& value) {
		...
		ParallelFor(
			...
			ZERO_SIZE, m_dataV.Height(),
			ZERO_SIZE, m_dataV.Depth(),
			[this, value](size_t i, size_t j, size_t k)
		{
		...
		ParallelFor(
			...
			ZERO_SIZE, m_dataW.Height(),
			ZERO_SIZE, m_dataW.Depth(),
			[this, value](size_t i, size_t j, size_t k)
		{
	...
	std::shared_ptr<VectorGrid3> FaceCenteredGrid3::Clone() const
	{
		return std::shared_ptr<FaceCenteredGrid3>(
			new FaceCenteredGrid3(*this), [](FaceCenteredGrid3* obj)
		{
			delete obj;
		});
	}
	...
	void FaceCenteredGrid3::OnResize(
		const Size3& Resolution,
		const Vector3D& GridSpacing,
		const Vector3D& Origin,
		const Vector3D& initialValue)
	...
	{
		size_t size =
			GetUSize().x * GetUSize().y * GetUSize().z +
			GetVSize().x * GetVSize().y * GetVSize().z +
			GetWSize().x * GetWSize().y * GetWSize().z;
		data->resize(size);
		size_t cnt = 0;
		
		m_dataU.ForEach([&](double value)
		{
			(*data)[cnt++] = value;
		});
		m_dataV.ForEach([&](double value)
		{
			(*data)[cnt++] = value;
		});
		m_dataW.ForEach([&](double value)
		{
			(*data)[cnt++] = value;
		});
	}
	...
	void FaceCenteredGrid3::SetData(const std::vector<double>& data)
	{
		assert(
			GetUSize().x * GetUSize().y * GetUSize().z +
			GetVSize().x * GetVSize().y * GetVSize().z +
			GetWSize().x * GetWSize().y * GetWSize().z == data.size());

		size_t cnt = 0;

		m_dataU.ForEachIndex([&](size_t i, size_t j, size_t k)
		{
			m_dataU(i, j, k) = data[cnt++];
		});
		m_dataV.ForEachIndex([&](size_t i, size_t j, size_t k)
		{
			m_dataV(i, j, k) = data[cnt++];
		});
		m_dataW.ForEachIndex([&](size_t i, size_t j, size_t k)
		{
	...
	FaceCenteredGrid3::Builder& FaceCenteredGrid3::Builder::WithResolution(size_t resolutionX, size_t resolutionY, size_t resolutionZ)
	{
	...
	FaceCenteredGrid3::Builder& FaceCenteredGrid3::Builder::WithGridSpacing(double gridSpacingX, double gridSpacingY, double gridSpacingZ)
	{
	...
	FaceCenteredGrid3::Builder& FaceCenteredGrid3::Builder::WithOrigin(const Vector3D& gridOrigin)
	{
		m_gridOrigin = gridOrigin;
		return *this;
	}

	FaceCenteredGrid3::Builder& FaceCenteredGrid3::Builder::WithOrigin(double gridOriginX, double gridOriginY, double gridOriginZ)
	{
	...
	FaceCenteredGrid3::Builder& FaceCenteredGrid3::Builder::WithInitialValue(double initialValX, double initialValY, double initialValZ)
	{
	...
	{
		return FaceCenteredGrid3(m_resolution, m_gridSpacing, m_gridOrigin, m_initialVal);
	}

	FaceCenteredGrid3Ptr FaceCenteredGrid3::Builder::MakeShared() const
	{
		return std::shared_ptr<FaceCenteredGrid3>(
			new FaceCenteredGrid3(m_resolution, m_gridSpacing, m_gridOrigin, m_initialVal),
