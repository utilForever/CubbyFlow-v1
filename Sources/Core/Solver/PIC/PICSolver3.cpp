...
namespace CubbyFlow
{
		...
		GridFluidSolver3(resolution, gridSpacing, gridOrigin)
	{
		...
		m_signedDistanceFieldID = grids->AddScalarData(std::make_shared<CellCenteredScalarGrid3::Builder>(), std::numeric_limits<double>::max());
	...
	void PICSolver3::MoveParticles(double timeIntervalInSeconds)
	{
		...
		ParallelFor(ZERO_SIZE, numberOfParticles, [&](size_t i)
		{
			...
			if ((domainBoundaryFlag & DIRECTION_BACK) && pt1.z <= boundingBox.lowerCorner.z)
			{
				pt1.z = boundingBox.lowerCorner.z;
				vel.z = 0.0;
			}
			if ((domainBoundaryFlag & DIRECTION_FRONT) && pt1.z >= boundingBox.upperCorner.z)
			{
	...
	void PICSolver3::BuildSignedDistanceField()
	{
		...
		sdf->ParallelForEachDataPointIndex([&](size_t i, size_t j, size_t k)
		{
			...
			searcher->ForEachNearbyPoint(pt, sdfBandRadius, [&](size_t, const Vector3D& x)
			{
	...
	{
		return std::shared_ptr<PICSolver3>(new PICSolver3(m_resolution, GetGridSpacing(), m_gridOrigin),
			[](PICSolver3* obj)
		{
			delete obj;
		});
