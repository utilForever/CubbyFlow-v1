/*************************************************************************
> File Name: FaceCenteredGrid2.cpp
...
namespace CubbyFlow
{
	FaceCenteredGrid2::FaceCenteredGrid2() :
		m_dataOriginU(0.0, 0.5), m_dataOriginV(0.5, 0.0),
		m_uLinearSampler(
	...
	double FaceCenteredGrid2::DivergenceAtCellCenter(size_t i, size_t j) const
	{
		...
		double bottomV = m_dataV(i, j);
		double topV = m_dataV(i, j + 1);
	...
	FaceCenteredGrid2::ConstScalarDataAccessor FaceCenteredGrid2::GetUConstAccessor() const
	{
	...
	FaceCenteredGrid2::ConstScalarDataAccessor FaceCenteredGrid2::GetVConstAccessor() const
	{
	...
	std::shared_ptr<VectorGrid2> FaceCenteredGrid2::Clone() const
	{
		return std::shared_ptr<FaceCenteredGrid2>(
			new FaceCenteredGrid2(*this), [](FaceCenteredGrid2* obj)
		{
			delete obj;
		});
	}
	...
	void FaceCenteredGrid2::OnResize(
		const Size2& Resolution,
		const Vector2D& GridSpacing,
		const Vector2D& Origin,
		const Vector2D& initialValue)
	...
	void FaceCenteredGrid2::SetData(const std::vector<double>& data)
	{
		...
		m_dataU.ForEachIndex([&](size_t i, size_t j)
		{
			m_dataU(i, j) = data[cnt++];
		});
		m_dataV.ForEachIndex([&](size_t i, size_t j)
		{
			m_dataV(i, j) = data[cnt++];
		});
	}
	...
	FaceCenteredGrid2::Builder& FaceCenteredGrid2::Builder::WithOrigin(const Vector2D& gridOrigin)
	{
	...
	{
		return FaceCenteredGrid2(m_resolution, m_gridSpacing, m_gridOrigin, m_initialVal);
	}

	FaceCenteredGrid2Ptr FaceCenteredGrid2::Builder::MakeShared() const
	{
		return std::shared_ptr<FaceCenteredGrid2>(
			new FaceCenteredGrid2(m_resolution, m_gridSpacing, m_gridOrigin, m_initialVal),
