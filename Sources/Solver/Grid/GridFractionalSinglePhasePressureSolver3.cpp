...
namespace CubbyFlow
{
						...
						for (size_t i = iBegin; i < iEnd; ++i)
						{
							...
							for (int z = 0; z < kernelSize[2]; ++z)
							{
								for (int y = 0; y < kernelSize[1]; ++y)
								{
									...
									{
										float w = kernels[0][x] * kernels[1][y] * kernels[2][z];
			...
			A->ParallelForEachIndex([&](size_t i, size_t j, size_t k)
			{
				...
				if (IsInsideSDF(centerPhi))
				{
					...
					if (i + 1 < size.x)
					{
						...
						(*b)(i, j, k) += uWeights(i + 1, j, k) * input.GetU(i + 1, j, k) * invH.x;
					}
					else
					{
					...
					if (i > 0)
					{
						...
						(*b)(i, j, k) -= uWeights(i, j, k) * input.GetU(i, j, k) * invH.x;
					}
					else
					{
					...
					if (j + 1 < size.y)
					{
						...
						(*b)(i, j, k) += vWeights(i, j + 1, k) * input.GetV(i, j + 1, k) * invH.y;
					}
					else
					{
					...
					if (j > 0)
					{
						...
						(*b)(i, j, k) -= vWeights(i, j, k) * input.GetV(i, j, k) * invH.y;
					}
					else
					{
					...
					if (k + 1 < size.z)
					{
						...
						(*b)(i, j, k) += wWeights(i, j, k + 1) * input.GetW(i, j, k + 1) * invH.z;
					}
					else
					{
					...
					if (k > 0)
					{
						...
						(*b)(i, j, k) -= wWeights(i, j, k) * input.GetW(i, j, k) * invH.z;
					}
					else
					{
	...
	const FDMLinearSystemSolver3Ptr& GridFractionalSinglePhasePressureSolver3::GetLinearSystemSolver() const
	{
		...
		const ScalarField3& fluidSDF)
	{
		...
		{
			m_fluidSDF[0](i, j, k) = static_cast<float>(fluidSDF.Sample(cellPos(i, j, k)));
		});
