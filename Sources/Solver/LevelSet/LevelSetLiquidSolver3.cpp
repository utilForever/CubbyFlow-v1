/*************************************************************************
> File Name: LevelSetLiquidSolver3.cpp
...
namespace CubbyFlow
{
		...
		GridFluidSolver3(resolution, gridSpacing, gridOrigin)
	{
		...
		m_signedDistanceFieldId = grids->AddAdvectableScalarData(std::make_shared<CellCenteredScalarGrid3::Builder>(), std::numeric_limits<double>::max());
	...
	{
		return GetGridSystemData()->GetAdvectableScalarDataAt(m_signedDistanceFieldId);
	}
	...
	void LevelSetLiquidSolver3::SetLevelSetSolver(const LevelSetSolver3Ptr& newSolver)
	{
