/*************************************************************************
> File Name: VolumeParticleEmitter.cpp
...
{
	pybind11::class_<VolumeParticleEmitter2, VolumeParticleEmitter2Ptr, ParticleEmitter2>(m, "VolumeParticleEmitter2")
	.def("__init__", [](VolumeParticleEmitter2& instance, pybind11::args args, pybind11::kwargs kwargs)
	{
	...
	if (args.size() >= 3 && args.size() <= 9)
	{
		...
		if (args.size() > 3)
		{
			initialVel = ObjectToVector2D(pybind11::object(args[3]));
		}
		if (args.size() > 4)
		{
	...
	if (kwargs.contains("seed"))
	{
		...
		new (&instance) VolumeParticleEmitter2(implicitSurface, bounds, spacing, initialVel, maxNumberOfParticles, jitter, isOneShot, allowOverlapping, seed);
	},
		R"pbdoc(
...
{
	pybind11::class_<VolumeParticleEmitter3, VolumeParticleEmitter3Ptr, ParticleEmitter3>(m, "VolumeParticleEmitter3")
	.def("__init__", [](VolumeParticleEmitter3& instance, pybind11::args args, pybind11::kwargs kwargs)
	{
		...
		if (args.size() >= 3 && args.size() <= 9)
		{
			...
			if (args.size() > 3)
			{
				initialVel = ObjectToVector3D(pybind11::object(args[3]));
			}
			if (args.size() > 4)
			{
				maxNumberOfParticles = args[4].cast<size_t>();
		...
		new (&instance) VolumeParticleEmitter3(implicitSurface, bounds, spacing, initialVel, maxNumberOfParticles, jitter, isOneShot, allowOverlapping, seed);
	},
		R"pbdoc(
