...
namespace CubbyFlow
{
	...
	Plane3::Plane3(const Vector3D& point0, const Vector3D& point1, const Vector3D& point2,
		const Transform3& transform_, bool isNormalFlipped_) :
		Surface3(transform_, isNormalFlipped_)
	{
	...
	Vector3D Plane3::ClosestNormalLocal([[maybe_unused]] const Vector3D& otherPoint) const
	{
		return normal;
	}
	...
	BoundingBox3D Plane3::BoundingBoxLocal() const
	{
		...
		return BoundingBox3D(Vector3D(dmax, dmax, dmax), Vector3D(dmax, dmax, dmax));
	}
	...
	{
		return std::shared_ptr<Plane3>(new Plane3(m_normal, m_point, m_transform, m_isNormalFlipped),
			[](Plane3* obj)
		{
			delete obj;
		});
