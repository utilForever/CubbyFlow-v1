...
namespace CubbyFlow
{
	...
	PCISPHSolver2::PCISPHSolver2(double targetDensity, double targetSpacing, double relativeKernelRadius) :
	...
	void PCISPHSolver2::AccumulatePressureForce(double timeIntervalInSeconds)
	{
		...
		for (unsigned int k = 0; k < m_maxNumberOfIterations; ++k)
		{
			...
			SPHSolver2::AccumulatePressureForce(x, ds.ConstAccessor(), p, m_pressureForces.Accessor());
	...
	{
		return PCISPHSolver2(m_targetDensity, m_targetSpacing, m_relativeKernelRadius);
	}

	PCISPHSolver2Ptr PCISPHSolver2::Builder::MakeShared() const
	{
		return std::shared_ptr<PCISPHSolver2>(
			new PCISPHSolver2(m_targetDensity, m_targetSpacing, m_relativeKernelRadius),
			[](PCISPHSolver2* obj)
		{
			delete obj;
		});
